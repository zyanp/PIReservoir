program runreservoir
  ...
  integer     (8), parameter :: N_resSQ = N_res*N_res, N_in = 1, N_out = 1
  ...
  real        (8), allocatable :: r1s ( :, : ), M ( :, : ), Mtemp ( : )
  ...
  do iab = 1, Na*Nb
    ...
    !$omp parallel
    !$omp do private(ith,M,Mtemp,W_in,i,r1,r2,r1s,ti,Z1,Z2,RR,RY,IPIV,WORK,INFO,X1p,X2p,meanX,varX,meanXp,varXp,filepath)
    do i_ens = 1, N_ens
    ...
    write ( filepath, '( A, I4.4, A, I3.3, A, I3.3, A )' ) &
      & '../data/Nres', N_res, '_regp1e-2_CLP_SoA_GammaM_J0J_Linear/MSE_CRC_NormalisedLorenz_ia', ia, '_ib', ib, '_traintest.dat'
