program runreservoir
  ...
  do iab = 1, Na*Nb
    ...
    !$omp parallel
    !$omp do private(ith,i_ens,i,k,M,Mtemp,sgnM,r,z,u,ti,JMbase,JM,unorm)
    do i_ens = 1, N_ens
      ith = omp_get_thread_num()
      allocate ( M ( N_res, N_res ), Mtemp ( N_resSQ ), sgnM ( N_res, N_res ) )
      state = vdrnggamma ( method, newstr, N_resSQ, Mtemp, shapek, aGamma, theta )
      M = reshape( Mtemp, (/ N_res, N_res /) )
      
      call random_number ( sgnM )
      sgnM = 2 * floor ( 2*sgnM ) - 1
    ...
    write ( filepath, '( A, I4.4, A, I3.3, A, I3.3, A )' ) &
      '../data/LyapunovExp_period1e4_withoutInput_symmetrisedGammaM_Nres', N_res, '_withoutBias_J0JLinear/ia', ia, '_ib', ib, '.dat'
