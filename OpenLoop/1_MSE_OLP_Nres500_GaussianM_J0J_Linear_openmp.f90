program runreservoir
  ...
  integer     (8), parameter :: i_w0 = 100, i_b0 = 0, lambexp = -2
  integer     (8), parameter :: Na = 40, Nb = 10, myrank = 1
  real        (8), parameter :: pi = dacos(-1.0d0), alpha = 0.2d0, lamb = dble(10**lambexp)
  real        (8), parameter :: w0 = dble(i_w0)*0.01d0, b0 = dble(i_b0)*0.01d0
  ...
  real        (8) :: r1 ( N_res ), Z1 ( N_res )
  real        (8) :: r2 ( N_res ), Z2 ( N_res )
  ...
  ! read training and test data for input
  ...
  do iab = 1, Na*Nb
    ...
    !$omp parallel
    !$omp do private(ith,W_in,i_ens,i,k,M,g1,g2,g3,g4,r1,r2,r1s,r2s,Z1,Z2,RR,RY,IPIV,WORK,INFO,ti,X1p,X2p)
    do i_ens = 1, N_ens
      ...
      ! state = vdRngGaussian ( method, newstr, N_resSQ, Mtemp, mu, std )
    ...
    write ( filepath, '( A, I4.4, A, I3.3, A, I3.3, A )' ) &
      '../data/MSE_OLP_Nres', N_res, '_GaussianM_J0J_Linear/ia', ia, '_ib', ib, '.dat'
